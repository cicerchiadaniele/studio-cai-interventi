# Studio CAI – Interventi (uso interno) v2.0
...
Versione 2.0 del 22/09/2026 (vedi CHANGELOG_v2.0.md).
...
- **Nel portale, prima dell'invio.** Il pannello sta sotto la Descrizione.
  Scelti condominio e tipo, l'app chiama il webhook
  `Interventi interno - Controllo doppioni` (scenario 7467174) e mostra le
  segnalazioni dello stesso tipo sul condominio negli ultimi 14 giorni. Appena
  si esce dal campo Descrizione il controllo si ripete inviando anche la
  descrizione (campo `messaggio`) e restano solo i problemi simili. All'invio,
  se c'è un possibile doppione, chiede conferma ("È un problema diverso,
  invia" / "Annulla"); se il controllo con la descrizione non è ancora
  concluso lo esegue in quel momento. Se il controllo non risponde,
