...
// v2.1: l'elenco resta sul dispositivo e si riscarica da Make solo se ha più di
// 7 giorni (o con il tasto "Aggiorna"): la sincronizzazione da Dropbox è mensile,
// scaricarlo a ogni apertura consumava crediti Make senza utilità.
const ELENCO_TS_KEY = "cs_elenco_condomini_ts";
const ELENCO_VALIDITA_MS = 7 * 24 * 3600 * 1000;
...
// Descrizione usata dal controllo doppioni (almeno 3 caratteri)
...
export default function StudioInterventi() {
  ...
  // v2.1: il controllo doppioni su app condomini + portale (ultimi 14 giorni)
  // parte una sola volta, quando si preme "Apri intervento", con condominio,
  // tipo e descrizione già compilati. Prima partiva a ogni cambio di campo
  // (2-4 chiamate Make per intervento).
  ...
  const aggiornaElenco = React.useCallback(async () => {
    ...
    try {
      ...
      setCondomini(lista);
      if (!PREVIEW) { scrivi(ELENCO_CACHE_KEY, lista); scrivi(ELENCO_TS_KEY, Date.now()); }
      setElencoStato("ok");
    } catch (e) {
  ...
  React.useEffect(() => {
    document.title = "Interventi – Studio CAI";
    const ts = Number(leggi(ELENCO_TS_KEY, 0)) || 0;
    const fresco = !PREVIEW && condomini.length > 0 && Date.now() - ts < ELENCO_VALIDITA_MS;
    if (fresco) setElencoStato("ok");
    else aggiornaElenco();
  }, [aggiornaElenco]); // eslint-disable-line
  ...
  const submit = async (forza = false) => {
    ...
    if (!forza) {
      ...
      } catch { remoti = []; } // se il controllo non risponde si invia comunque
      setControllo(false);
      setSending(false);
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h1 className="font-display font-semibold text-xl sm:text-2xl text-neutral-900 truncate">Studio CAI</h1>
          ...
          {result && (
            <motion.div
              ...
              className={cn(
                ...
                result.ok ? "bg-gradient-to-br from-brand/8 to-white border-brand/30" : "bg-gradient-to-br from-red-50 to-white border-red-300"
              )}
            >
              <div className="flex items-start gap-3">
                <div className={cn("flex-shrink-0 w-11 h-11 rounded-2xl flex items-center justify-center",
                  result.ok ? "bg-brand/12" : "bg-red-100")}>
                  {result.ok ? <CheckCircle2 className="w-6 h-6 text-brand-dark" /> : <AlertCircle className="w-6 h-6 text-red-700" />}
                </div>
                <div className="flex-1 min-w-0">
                  {result.ok ? (
                    <>
                      ...
                      {result.conferma === "inviata" && (
                        <p className="mt-1.5 text-xs text-brand-dark flex items-center gap-1.5">
                ...
                <button onClick={() => setResult(null)} className="p-1.5 rounded-lg hover:bg-white/60" aria-label="Chiudi">
                  <X className={cn("w-4 h-4", result.ok ? "text-brand-dark" : "text-red-700")} />
                </button>
              </div>
            </motion.div>
          )}
        ...
        <div className="bg-white rounded-3xl shadow-lift ring-1 ring-neutral-200/80 overflow-hidden">
          ...
          <div className="p-5 sm:p-7 divide-y divide-neutral-100 [&>*]:py-7 [&>*:first-child]:pt-0 [&>*:last-child]:pb-0">
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-1">
              ...
              >
                {sending ? <><Loader2 className="w-5 h-5 animate-spin" />{controllo ? "Controllo doppioni…" : "Invio in corso…"}</> : <><Send className="w-5 h-5" />Apri intervento</>}
              </button>
              <button
                type="button"
                disabled={sending}
                onClick={() => { setForm(formVuoto(form.operatore)); setTouched({}); setDoppione(null); setRifNonInviato(null); }}
            ...
            <li key={v.id} className="py-3 flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-neutral-500 mt-0.5 flex flex-wrap gap-x-2">
                  ...
                  {v.conferma === "inviata" && <span className="text-brand-dark font-semibold">conferma inviata</span>}
              ...
              <span className={cn(
                ...
                ok ? "bg-brand/8 text-brand-dark ring-1 ring-brand/20" : "bg-amber-50 text-amber-800 ring-1 ring-amber-200"
              )}>
  ...
  return (
    <div ref={box} className="relative">
      <p className="text-sm font-semibold text-neutral-700 flex items-center gap-1 mb-1.5">
        ...
        {stato !== "caricamento" && (
          <button type="button" onClick={onRiprova} title="Scarica di nuovo l'elenco condomini"
            className="font-normal text-xs text-[var(--brand)] underline underline-offset-2 hover:opacity-80">
            Aggiorna elenco
          </button>
        )}
      </p>
