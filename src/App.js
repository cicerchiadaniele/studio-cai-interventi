...
import {
  ...
  Zap, KeyRound, Trees, Droplets, Flame, Accessibility, Clock, ClipboardList, History
} from "lucide-react";
...
const TIPI_INTERVENTO = [
  { value: "Antennista", label: "Antennista", icon: Antenna },
  { value: "Ascensore", label: "Ascensore", icon: ArrowUpDown },
  { value: "Cancelli Elettrici", label: "Cancelli elettrici", icon: Fence },
  { value: "Disinfestazioni/Derattizzazioni", label: "Disinfestazioni", icon: Bug },
  { value: "Edilizia", label: "Edilizia", icon: BrickWall },
  { value: "Elettricista", label: "Elettricista", icon: Zap },
  { value: "Fabbro", label: "Fabbro", icon: KeyRound },
  { value: "Giardinaggio", label: "Giardinaggio", icon: Trees },
  { value: "Idraulico", label: "Idraulico", icon: Droplets },
  { value: "Impianto di Riscaldamento", label: "Riscaldamento", icon: Flame },
  { value: "Montascale", label: "Montascale", icon: Accessibility },
];
...
export default function StudioInterventi() {
  ...
  const submit = async (forza = false) => {
    ...
    if (!forza) {
      ...
      if (prec) { setDoppione(prec); return; }
    }
    setDoppione(null);
    setSending(true);
    setResult(null);
    ...
    try {
      ...
      setResult({ ok: true, ticket, condominio: clean.condominio, tipo: clean.subcategoria });
      registraInvio({
          ...
          <div className="p-5 sm:p-7 divide-y divide-neutral-100 [&>*]:py-7 [&>*:first-child]:pt-0 [&>*:last-child]:pb-0">
            ...
            <AnimatePresence>
              {doppione && (
                ...
                >
                  <p className="font-semibold text-amber-900">Intervento simile già aperto oggi</p>
                  <p className="text-sm text-amber-800 mt-1">
                    Da questo dispositivo, nelle ultime 24 ore, è partito un intervento {doppione.tipo} per {doppione.condominio}
                    {" "}(<span className="font-mono font-semibold">{doppione.ticket}</span>).
                  </p>
