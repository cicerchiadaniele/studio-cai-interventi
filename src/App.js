...
import {
  ...
  Zap, KeyRound, Trees, Droplets, Flame, Accessibility, Clock, ClipboardList, History, Copy, Info
} from "lucide-react";
...
const BUILD_DATE_LABEL = "17/09/2026";
...
async function controllaDoppioni({ condominio, subcategoria, scala, interno }) {
  if (PREVIEW) {
  ...
  try {
    const res = await fetch(CONTROLLO_URL, {
      method: "POST",
      signal: ctrl.signal,
      body: new URLSearchParams({ condominio, subcategoria, scala: scala || "", interno: interno || "", versione: APP_TAG }),
    });
    const json = JSON.parse(await res.text());
...
export default function StudioInterventi() {
  ...
  // Controllo doppioni su app condomini + portale (ultimi 14 giorni)
  const [verifica, setVerifica] = useState({ stato: "vuoto", chiave: "", trovati: [] });
  React.useEffect(() => {
    if (!form.condominio || !form.subcategoria) { setVerifica({ stato: "vuoto", chiave: "", trovati: [] }); return; }
    const k = `${form.condominio}|${form.subcategoria}`;
    let annullato = false;
    setVerifica((v) => ({ ...v, stato: "carico", chiave: k }));
    const t = setTimeout(async () => {
      try {
        const trovati = await controllaDoppioni({ condominio: form.condominio, subcategoria: form.subcategoria, scala: form.scala, interno: form.interno });
        if (!annullato) setVerifica({ stato: "ok", chiave: k, trovati });
      } catch {
        if (!annullato) setVerifica({ stato: "errore", chiave: k, trovati: [] });
      }
    }, 500);
    return () => { annullato = true; clearTimeout(t); };
    // scala e interno non rilanciano la ricerca: servono solo a ordinare meglio
  }, [form.condominio, form.subcategoria]); // eslint-disable-line
  ...
  const submit = async (forza = false) => {
    ...
    if (!forza) {
      ...
      const remoti = verifica.chiave === `${clean.condominio}|${clean.subcategoria}`
        ? verifica.trovati.filter((x) => x.livello === "alta") : [];
    ...
    <div className="relative min-h-screen w-full bg-paper bg-noise text-neutral-900" style={cssVars}>
      ...
      <main className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="bg-white rounded-3xl shadow-lift ring-1 ring-neutral-200/80 overflow-hidden">
          <div className="relative overflow-hidden bg-gradient-to-br from-[var(--brand)] via-[var(--brand-dark)] to-[var(--brand-deep)] px-5 sm:px-7 py-5">
            ...
            <div className="relative flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">I campi con * sono obbligatori</p>
              </div>
            </div>
          </div>
                ...
                {TIPI_INTERVENTO.map(({ value, label, icon: Icona }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[13px] sm:text-sm font-semibold leading-tight min-w-0 break-words">{label}</span>
                    </button>
                  );
                })}
              </div>
              ...
              {doppione && (
                ...
                >
                  ...
                  {doppione.remoti?.length > 0 && (
                    <p className="text-sm text-amber-800 mt-1">
                      Negli ultimi 14 giorni c'è già {doppione.remoti.length === 1 ? "una segnalazione" : `${doppione.remoti.length} segnalazioni`} dello stesso tipo su questo condominio:{" "}
  ...
  return (
    <section className="mt-6 bg-white rounded-3xl ring-1 ring-neutral-200/80 shadow-soft p-5 sm:p-6">
      <div className="flex items-baseline justify-between gap-3">
        ...
        <span className="text-xs text-neutral-500 tabular-nums">{invii.length}/{INVII_MAX}</span>
      </div>
  ...
  if (verifica.stato === "carico") {
    return (
      <p className="mt-4 text-xs text-neutral-500 flex items-center gap-1.5">
        <Loader2 className="w-3.5 h-3.5 animate-spin" /> Controllo segnalazioni recenti su questo condominio…
      </p>
    );
  }
  ...
  if (!verifica.trovati.length) {
    return (
      <p className="mt-4 text-xs text-emerald-700 flex items-center gap-1.5">
        <CheckCircle2 className="w-3.5 h-3.5" /> Nessuna segnalazione simile negli ultimi 14 giorni.
      </p>
    );
  }
  const Voce = ({ x }) => (
      ...
      {stesso.length > 0 ? (
        <>
          <p className="text-sm font-semibold text-amber-900 flex items-center gap-1.5">
            <Copy className="w-4 h-4" /> Possibile doppione: {stesso.length === 1 ? "già segnalato" : `già segnalato ${stesso.length} volte`} negli ultimi 14 giorni
          </p>
